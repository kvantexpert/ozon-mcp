...
Сейчас systemd использует uvx --from ozon-mcp-ru без фиксации версии. Это сохраняет текущее поведение, но при восстановлении может быть установлена более новая версия пакета. Перед переносом production рекомендуется отдельно зафиксировать точную версию ozon-mcp-ru.
